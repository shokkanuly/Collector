...
def load_resources():
    ...
    if not has_labels:
        # Never guess the class list: a wrong silent fallback makes every
        # prediction index map to the wrong letter.
        st.error("label_map.joblib not found — run train.py first.")
        st.stop()
